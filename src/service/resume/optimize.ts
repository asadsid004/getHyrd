...
import { aiAnalyzeSchema } from "./analyse";

export const optimizeResumeTextData = async (
    role: string,
    description: string,
...
export const optimizeResumeAnalysisBasedData = async (
    resumeData: z.infer<typeof ResumeSchema>,
...
`.trim();
    ...
    const result = await generateObject({
        ...
        schema: ResumeSchema,
        temperature: 0.2, // Lower temperature for consistency
        prompt: `You are an expert resume optimizer. Optimize ONLY the text content of the resume based on the analysis data.
...
2. WHAT YOU SHOULD OPTIMIZE (ONLY THESE):
   - summary: Rewrite to align with the analysis data
   - experience[].description: Rewrite with action verbs and metrics
   - experience[].achievements: Rewrite to be more impactful
   - projects[].description: Rewrite to emphasize relevant tech
   - projects[].highlights: Rewrite to show impact
...
3. OPTIMIZATION GUIDELINES:
   ...
   - Match keywords from job description naturally
   - Keep descriptions concise but impactful
   - Focus on the analysis data relevant achievements
